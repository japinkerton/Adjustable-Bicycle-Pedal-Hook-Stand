//Diameter of circular base plate
BASE_DIA = 200; 
//Thickness of circular base plate
BASE_THICK = 8; 
//Diameter of 4 holes in base plate
BASE_HOLE_DIA = 20; 
//how far out to place the relief holes in the base.  1/2 would place them halfway between the center and outer edge.  Larger ratios will place the holes farther out.
BASE_HOLE_DIST_RATIO = 2/3;  
// Radius of relief cuts in the base plate
BASE_RELIEF_RAD = 45; 

//How far the stem extends from the base plate
STEM_HEIGHT = 70; 
//Outer diameter of the stem
STEM_DIA = 70; 
//How much of the stem should be taken up by the threads (.9 means 90% of the diameter of the stem will be used by the threads.
STEM_THREAD_RATIO = .9;
//Thread pitch of adjustable stem 
STEM_THREAD_PITCH = 7; 

// Inner X axis (width) of the pedal support cradle
CRADLE_X = 40;  
// Inner Y axis (depth) of the pedal support cradle
CRADLE_Y = 20; 
// Inner Z axis (height) of the pedal support cradle 
CRADLE_Z = 48;  
// Wall thickness of the pedal support cradle
CRADLE_WALL_THICK = 3; 
//Width of the slot in the pedal support cradle.
CRADLE_SLOT_WIDTH = 20; 
//Size of the nut below the cradle
CRADLE_NUT_WIDTH = 70;
...
module base(dia,thick,holeDia,distRatio,stemHeight,stemDia,reliefRad,pitch,threadRatio)
{
    holeOffset = dia/2 * distRatio;
    threadDia = stemDia*threadRatio;
    ScrewHole(threadDia,stemHeight+thick,pitch=pitch)
    {
        difference()
        {
            cylinder(thick,dia/2,dia/2);
            translate([holeOffset,0,0])   cylinder(thick,holeDia/2,holeDia/2);
            translate([-holeOffset,0,0])   cylinder(thick,holeDia/2,holeDia/2);
            translate([0,holeOffset,0])   cylinder(thick,holeDia/2,holeDia/2);
            translate([0,-holeOffset,0])   cylinder(thick,holeDia/2,holeDia/2);
            rotate([0,0,45]) translate([dia/2,0,0]) cylinder(thick,reliefRad,reliefRad);
            rotate([0,0,135]) translate([dia/2,0,0]) cylinder(thick,reliefRad,reliefRad);
            rotate([0,0,-45]) translate([dia/2,0,0]) cylinder(thick,reliefRad,reliefRad);                    
            rotate([0,0,-135]) translate([dia/2,0,0]) cylinder(thick,reliefRad,reliefRad);                    
        }
        translate([0,0,thick])cylinder(stemHeight,stemDia/2,stemDia/2);   
    }
}

module cradle(x,y,z,wallThick,slotWidth,stemDia,stemThreadHeight,pitch,ratio,nutHeight,nutWidth)
{

    threadDia = stemDia*ratio;
    ScrewThread(threadDia,stemThreadHeight,pitch=pitch);
    translate([0,0,stemThreadHeight])
        cylinder_outer(nutHeight,nutWidth/2,6);
    translate([-(x+wallThick)/2,-(y+wallThick)/2,stemThreadHeight+nutHeight])
    {
        difference()
        {
            cube([x+wallThick*2,y+wallThick*2,z]);
            translate([wallThick,wallThick,0])   cube([x,y,z]);
            translate([wallThick + (x-slotWidth)/2,0,0]) cube([slotWidth,wallThick,z]);
            //translate([x/2+wallThick,y+wallThick*2,z/2]) rotate([90,0,0]) cylinder(wallThick,x/2,x/2);
        } 
    }    
}
